class ProvisionStripePaymentSettingsToAllUsers < ActiveRecord::Migration[5.1]
  def up
    communities_with_stripe_ids    = Community.connection.select_values("SELECT community_id FROM payment_settings WHERE payment_gateway = 'stripe'")
    ...
    WHERE ps.id IS NULL
    SQL
    communities_without_paypal_ids = Community.connection.select_values communities_without_paypal_query
    ...
    Community.connection.execute provision_query
  end
end
